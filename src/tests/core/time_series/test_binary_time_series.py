...
from src.ontime.core.time_series.binary_time_series import BinaryTimeSeries
...
class TestBinaryTimeSeries(unittest.TestCase):
    def setUp(self):
        ...
        my_dict = {"times": keys, "values": values}
        # Create a pandas dataframe from the dictionary
        df = pd.DataFrame.from_dict(my_dict)
        df.set_index("times", inplace=True)
        self.df = df

    def test_constructor__creation_from_dataframe__should_create_object_with_correct_data(
        self,
    ):
