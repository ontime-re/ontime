...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_tensor = torch.tensor(self.data_array[index], dtype=torch.float32).transpose(-1, -2)
        label_tensor = torch.tensor(self.labels_array[index], dtype=torch.float32).transpose(-1, -2)
